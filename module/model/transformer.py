...
class MultiHeadSelfAttention(layers.Layer):
    def __init__(self, embed_dim, num_heads=8):
        ...
        if embed_dim % num_heads != 0:
            raise ValueError(
                'embedding dimension = {} should be divisible by number of head = {}'.format(embed_dim, num_heads)
            )
...
class TokenAndPositionEmbedding(layers.Layer):
    def __init__(self, maxlen, vocab_size, embed_dim, pretrained_embedding=None):
        ...
        if pretrained_embedding is not None:
            self.token_emb = layers.Embedding(input_dim=vocab_size, output_dim=embed_dim, weights=[pretrained_embedding]
...
class TransformerClassifier(object):
    def __init__(self, classes, config, pretrain_embedding):
        self.models = {}
        self.classes = classes
        self.num_class = len(classes)
        self.config = config
        self.model = self._build(pretrain_embedding)

    def _build(self, pretrain_embedding):
        ...
        embedding_layer = TokenAndPositionEmbedding(maxlen, vocab_size, embedding_dim, pretrain_embedding)
